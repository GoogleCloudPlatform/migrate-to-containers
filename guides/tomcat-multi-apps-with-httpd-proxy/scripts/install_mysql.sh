...
# Create the flowcrm user
sudo mysql -e "CREATE USER 'flowcrm'@'%' IDENTIFIED BY 'flowcrm';"
# Create a new database for flowcrm
sudo mysql -e "CREATE DATABASE flowcrm;"
# Grant all for flowcrm user on flowcrm database
sudo mysql -e "GRANT ALL PRIVILEGES ON flowcrm.* TO 'flowcrm'@'%' WITH GRANT OPTION;"
